...
- `app/services/` — supporting logic: `human_pose`, `text`, `image`, `transition`, `weather`, `fps`, `controller`/`controller_mapping` (BLE gamepads), `sandbox`/`script_store` (scripted animations), `settings_store`/`chat_session_store` (persistence), `fonts/`.
...
- **In-UI Claude chat** (`app/infrastructure/chat.py`): the backend runs the agentic tool-use loop itself as the MCP client and streams NDJSON back to `/chat`. Needs `ANTHROPIC_API_KEY`. Default model `claude-opus-4-8` (`ANTHROPIC_MODEL` / per-request override; allowed: `claude-opus-4-8`, `claude-sonnet-4-6`). Conversations persist to `state/chat_sessions/`.
