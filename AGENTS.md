...
- **MCP server** (`app/infrastructure/mcp_server.py`, `ENABLE_MCP=true` by default): a `FastMCP` instance exposing the display to AI agents as tools (`get_display` returns the panel as both a PNG image and ASCII art, `set_mode`, board drawing, `run_script`, etc.). The same object backs two consumers: the in-UI chat (in-process) and the external HTTP `/mcp` endpoint. **`/mcp` is only mounted when `MCP_AUTH_TOKEN` is set** and is bearer-gated + DNS-rebinding-protected; the in-UI chat works without it.
- **In-UI AI chat** (`app/infrastructure/chat.py` + `chat_openai.py`): the backend runs the agentic tool-use loop itself as the MCP client and streams NDJSON back to `/chat`. Three providers, keyed by env var: Anthropic (`ANTHROPIC_API_KEY`; Claude Opus 4.8 / Sonnet 5 / Fable 5 / Haiku 4.5), OpenAI (`OPENAI_API_KEY`; GPT-5.5, GPT-5.4, GPT-5.4 mini/nano), and OpenRouter (`OPENROUTER_API_KEY`; DeepSeek V4 Flash, GLM-5.2). The model registry lives in `chat.MODELS`; models whose provider has no key show as unavailable in the UI, and a `supports_vision` flag (set on the Claude and GPT-5.x entries) gates whether `get_display`'s PNG is forwarded to the model — vision models get the image (ASCII dropped to save tokens), others fall back to the ASCII. Default model `claude-opus-4-8` (`ANTHROPIC_MODEL` override). **The model is locked per conversation** (histories are provider-native; start a new chat to switch). Token usage and cost are accumulated per turn and per session (OpenRouter cost comes from the API's usage accounting, not a static rate table). Conversations persist to `state/chat_sessions/`.
...
- **Config via `.env`** (loaded by `python-dotenv`). Common keys:
  ...
  - Controllers: `PRIMARY_CONTROLLER_ADDRESS`/`PRIMARY_CONTROLLER_NAME`, `SECONDARY_CONTROLLER_ADDRESS` (no defaults — a hub with neither an address nor a name hint matches no device and stays idle). BLE link tuning applied after each connect: `CONTROLLER_SUPERVISION_TIMEOUT_MS` (default 2000; 0 disables the LE Connection Update), `CONTROLLER_CONN_MIN_INTERVAL_MS`/`CONTROLLER_CONN_MAX_INTERVAL_MS` (default 15/30) — a longer supervision timeout lets weak controllers ride through brief signal fades instead of dropping.
