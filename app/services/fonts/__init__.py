...
class FontDefinition:
    """A loaded bitmap font: glyph arrays plus optional monospace cell width."""
...
def _load_font_families() -> dict[str, dict[int, dict[str, FontDefinition]]]:
    ...
    for path in sorted(_font_data_dir().glob("*.json")):
        ...
        glyphs = {char: np.array(rows, dtype=np.uint8) for char, rows in raw_glyphs.items()}
...
def get_font_definition(family: str, size: int, style: str) -> FontDefinition:
    ...
    if size not in family_fonts:
        raise KeyError(
            f"Unknown font size {size} for family '{family}'. Available: {tuple(sorted(family_fonts.keys()))}"
        )

    size_fonts = family_fonts[size]
    if style not in size_fonts:
        raise KeyError(
            f"Unknown font style '{style}' for family '{family}' size {size}. Available: {tuple(sorted(size_fonts.keys()))}"
        )
