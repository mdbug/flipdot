...
_TETRIS_SONG_IDX = next(i for i, s in enumerate(AutoDrum.SONGS) if s["name"] == "TETRIS")
...
class Tetris(AutoDrum):
    ...
    * Arms crossed          → exit to menu (handled by main loop as usual)
    ...
    def __init__(self, width: int, height: int, mode_manager: ModeManager) -> None:
        ...
        self._jingle_events: list[tuple[float, str]] = []  # (due, note) death-jingle queue
    ...
    def _trigger_game_over(self, now):
        ...
        self._jingle_events = [(now + i * 0.08, note) for i, note in enumerate(notes)]
    ...
    def _render_player_board(self):
        ...
        # Locked cells
        for r, cc in zip(*np.nonzero(self._player["board"]), strict=True):
        ...
        if flash and self._player["flash_ticks"] % 2 == 0:
            for r in flash:
                bg[top + r * c : top + (r + 1) * c, x0 : x0 + t["ncols"] * c] = 1
    ...
    def _ai_score_board(board, ncols, nrows):
        ...
        return -aggregate_height * 0.51 - holes * 6 - bumpiness * 1.8 - wells * 1.2
    ...
    def _ai_drop_board(self, board, cells, col):
        ...
        if cleared:
            nb = np.vstack([np.zeros((cleared, t["ncols"]), dtype=bool), nb[~full]])
        return nb, cleared
                ...
                for ncells in next_rots:
                    ...
                    for ncol in range(0, t["ncols"] - n_max_dc):
                        ...
                        if res2 is None:
                            continue
                        b2, c2 = res2
                        s2 = c2 * 100 + self._ai_score_board(b2, t["ncols"], t["nrows"])
                        if best_next is None or s2 > best_next:
                            best_next = s2
                if best_next is None:
                    # Next piece would have nowhere to go: near-certain death
                    score = c1 * 100 + self._ai_score_board(b1, t["ncols"], t["nrows"]) - 1000
                else:
    ...
    def _handle_ai(self, now):
        ...
        if p["rot_idx"] != target_rot and p["row"] >= 0 and now - self._ai_rotate_cooldown >= 0.15:
        ...
        if (
            p["row"] >= self.AI_HARD_DROP_MIN_ROW
    ...
    def _handle_gestures(self, pose_results, now):
        ...
        if self._player["game_over"]:
            ...
            if (
                self._player["game_over_time"] is not None
        ...
        if person_present:
            ...
            if self._last_controller_input_time is not None and (
    ...
    def queue_controller_move(self, dx: int) -> None:
        """Queue a horizontal piece move from the controller (``dx`` columns)."""
        if int(dx) == 0:
            return
        self._mark_controller_input()
        self._pending["move"] = -1 if int(dx) < 0 else 1

    def queue_controller_rotate_ccw(self) -> None:
        """Queue a counter-clockwise rotation from the controller."""
        self._mark_controller_input()
        self._pending["rotate"] = -1

    def queue_controller_rotate_cw(self) -> None:
        """Queue a clockwise rotation from the controller."""
        self._mark_controller_input()
        self._pending["rotate"] = 1

    def queue_controller_hard_drop(self) -> None:
        """Queue a hard drop from the controller."""
        self._mark_controller_input()
        self._pending["hard_drop"] = True
