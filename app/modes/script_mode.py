...
class ScriptMode:
    def __init__(self, width: int, height: int, store: Optional[ScriptStore] = None) -> None:
        self.width = width
        self.height = height
        self._store = store or ScriptStore()
        self._script: Optional[SandboxedScript] = None
        self._name: str = ""
        self._start_time = 0.0

    # --- render loop -----------------------------------------------------

    def get_frame(self, *_args, **_kwargs) -> np.ndarray:
    ...
    def _replace(self, script: Optional[SandboxedScript], name: str) -> None:
