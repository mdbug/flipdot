    ...
    * row  h-1     : sequencer step cursor (inherited convention)
    ...
    def _pose_to_mask(self, pose_results: Any) -> np.ndarray | None:
        ...
        if seg is not None:
            ...
            s = min(hh, ww)
            r0, c0 = (hh - s) // 2, (ww - s) // 2
            img = Image.fromarray(((arr[r0 : r0 + s, c0 : c0 + s] > 0.5) * 255).astype(np.uint8))
        ...
        if all(i in pts for i in quad):
            for t in np.linspace(0.0, 1.0, max(2, m["mh"] // 2)):
                left = tuple(
                    pts[self._L_SHOULDER][k] + (pts[self._L_HIP][k] - pts[self._L_SHOULDER][k]) * t
                    for k in (0, 1)
                )
                right = tuple(
                    pts[self._R_SHOULDER][k] + (pts[self._R_HIP][k] - pts[self._R_SHOULDER][k]) * t
                    for k in (0, 1)
                )
                line(left, right)
