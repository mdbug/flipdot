...
class AutoDrum:
    ...
    * 'bg_step'– name of a method advancing an animated background once
    ...
    MELODY_LONG_DECAY = [0.15, 0.12, 0.10, 0.09, 0.08, 0.07, 0.06, 0.05, 0.05, 0.04, 0.04, 0.03]
    ...
    SONGS = [
        ...
        },
        {
            ...
            "melody": {
                "pitches": (("g", 67), ("e", 64), ("d", 62), ("c", 60), ("b", 59)),
                "long": ("c", "b"),
            },
        },
        {
            ...
            "melody": {
                "pitches": (("c5", 72), ("bb4", 70), ("ab4", 68), ("g4", 67)),
                "long": ("c5", "ab4"),
            },
        },
        {
        ...
        if "melody" in song:
            ...
            for name in m.get("long", ()):
                self.instruments[name + "_long"] = {
                    "area": self.instruments[name]["area"],
                    "density": 1.0,
                    "voice": True,
                    "decay": list(self.MELODY_LONG_DECAY),
                }
                self._melody_names.append(name + "_long")

    def next_song(self) -> None:
        """Advance to the next song in the rotation and reload its sequencer state."""
    ...
    def _tetris_background(self):
        ...
        for i, r in enumerate(range(nrows - 1, max(nrows - 3, 0), -1)):
            gaps = self.rng.choice(ncols, size=min(2 + 2 * i, ncols), replace=False)
    ...
    def _tet_eval(self, cells, col):
        ...
        if cleared:
            b = np.vstack([np.zeros((cleared, t["ncols"]), dtype=bool), b[~full]])
        ...
        score = 10 * holes + int(heights.sum()) + 2 * int(heights.max()) - 30 * cleared
        return score, row
    ...
    def _tetris_step(self, now):
        ...
        if t["flash"]:
            ...
            if t["flash_ticks"] >= 2:  # blinked → collapse the rows
                ...
                keep[t["flash"]] = False
                t["board"] = np.vstack(
                    [np.zeros((len(t["flash"]), t["ncols"]), dtype=bool), t["board"][keep]]
                )
                t["flash"] = []
                self._tet_spawn()
        else:
    ...
    def _tetris_render(self):
        ...
        flash = set(t["flash"])
        for r, cc in zip(*np.nonzero(t["board"]), strict=True):
    ...
    def _voice_hit(self, name, now):
        ...
        if inst["decay"]:
            self._voice_decay = [
                (now + (i + 1) * self.DECAY_TICK, d) for i, d in enumerate(inst["decay"])
            ]
            # Final zero-density tick sweeps up the last shimmer dots.
            self._voice_decay.append((now + (len(inst["decay"]) + 1) * self.DECAY_TICK, 0.0))
    ...
    def _tick_voice(self, now):
        ...
        while self._voice_decay and self._voice_decay[0][0] <= now:
            ...
            if self._voice_shimmer is not None:
                ...
                self._voice_shimmer = None
            if density > 0:
                self._voice_shimmer = (self.rng.random((r1 - r0, c1 - c0)) < density).astype(
                    np.uint8
                )
                self.state[r0:r1, c0:c1] ^= self._voice_shimmer
    ...
    def get_frame(self, pose_results: Any) -> Frame:
        """Advance the sequencer/animation and return the rendered panel frame."""
        now = time.time()
        song = self.SONGS[self.song_index]
        ...
        if self._skip_hold_start is not None:
            progress = min(
                int((now - self._skip_hold_start) / self.SKIP_HOLD_TIME * self.width), self.width
            )
            frame[0, :progress] ^= 1
