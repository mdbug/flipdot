...
- **Controllers (optional):** two [IINE GameBrick Mini retro controllers](https://iine.store/products/iine-gamebrick-mini-retro-controller) — Bluetooth gamepads (D-pad + A/B) — read via `evdev`, one per player for the two-player games (pong, tank). Any Bluetooth HID gamepad works. A systemd unit disables Bluetooth ERTM, which otherwise causes multi-second input freezes, and the connection tuning in `CONTROLLER_*` env vars helps these low-power pads ride through signal fades.
