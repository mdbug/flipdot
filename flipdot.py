...
while True:
    ...
    if now.hour < 7 or now.hour >= 24:
        ...
        if pose_results.pose_landmarks:
            if mode_manager.mode == mode_manager.MODE_POSE:
                ...
                if mode_manager.mode != ModeManager.MODE_MENU and mode_manager.mode != ModeManager.MODE_PAINT:
                    mode_manager.set_mode(ModeManager.MODE_POSE)

            if mode_manager.mode == ModeManager.MODE_POSE or mode_manager.mode == ModeManager.MODE_CLOCK:
                click_menu, finger_x, finger_y = human_pose.is_right_index_in_top_right_corner(pose_results)
                if click_menu:
    ...
    if current_time - last_print_time >= PRINT_INTERVAL:
        ...
        print(f"\rMode: {mode_manager.mode} | Eyes visible: {eyes_visible} {reason} | Dist: {estimated_distance_str} | Finger: {finger_x}, {finger_y} | FPS: {stats['fps']:.1f} | Avg: {stats['avg_fps']:.1f} | ", end='', flush=True)
